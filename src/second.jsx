    ...
    <div className="time-container">
      <div className="header">
        <div className="card">
          ...
          <div className="days">
            <p ><a onClick={()=>{setDuration("daily")}} >Daily</a></p>
            <p><a onClick={()=>{setDuration("weekly")}}>Weekly</a></p>
            <p><a onClick={()=>{setDuration("monthly")}}>Monthly</a></p>
          </div>
        </div>
      </div>
      {data.map((item) =>{
         return <div className={((item.title==="Work"||item.title==="Play"||item.title==="Study")? 'card-context' : 'second-row') + ` ${item.title}`} key={item.title}>
              <div className="context">
                <div className = "context-header">
                  <span className="activity">{item.title}</span>
                ...
                <div className ="context-time">
                  <p className="current">
                  {duration == "daily" ? item.timeframes.daily.current : 
                  (duration == "monthly") ? item.timeframes.monthly.current : 
                  item.timeframes.weekly.current }hrs
                  </p>
                  <p className="previous">Last { }
                  {duration == "daily" ? "Day" : (duration == "monthly") ?  "Month": "Week"} - { }
                  {duration == "daily" ? item.timeframes.daily.previous : 
                  (duration == "monthly") ? item.timeframes.monthly.previous : 
              ...
              {
                  {
                    'Work': <img className="badge" src={Work} alt="React Logo" />,
                    'Play': <img className="badge" src={Play} alt="React Logo" />,
                    'Study': <img className="badge" src={Study} alt="React Logo" />,
                    'Exercise': <img className="badge" src={Exercise} alt="React Logo" />,
                    'Social': <img className="badge" src={Social} alt="React Logo" />,
                    'Self Care': <img className="badge" src={Self} alt="React Logo" />,
                  }[item.title]
              }
            </div>
      })}
            
            {/* <div className="card-context play">
              <div className="context ">
                <div className = "context-header">
                    <span className="activity">{data[1].title}</span>
                    <span className="fav-icon">
                      <svg width="21" height="5" xmlns="http://www.w3.org/2000/svg"><path d="M2.5 0a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5Zm8 0a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5Zm8 0a2.5 2.5 0 1 1 0 5 2.5 2.5 0 0 1 0-5Z" fill="#BBC0FF" fill-rule="evenodd"/></svg>
                    </span>
                  </div>
